...
const IndividualJob = ({ job }) => {
  ...
  const handleDisableJob = async () => {
    try {
      const url = `https://jobportalsdpps18-s15-04-90053-31880.up.railway.app/v1/api/job/disableJob?id=${job.id}&userName=${user.username}`;
  ...
  const handleEnableJob = async () => {
    try {
      const url = `https://jobportalsdpps18-s15-04-90053-31880.up.railway.app/v1/api/job/enableJob?id=${job.id}&vacancy=${newVacancy}&userName=${user.username}`;
      const response = await API.put(url);
      if (response.status === 200) {
        setIsPublish(true);
      }
  ...
  const handleUpdate = async (updatedJob) => {
    ...
    try {
      const url = `https://jobportalsdpps18-s15-04-90053-31880.up.railway.app/v1/api/job/updateJob/${updatedJob.id}`;
